...
module.exports = function(mongodb, mongourl) {
    return function(req, res) {
        ...
        .then(db => {
            return (_db = db).collection('settings').find({id: req.jwt.payload.user_id}).toArray();
        })
        .then(accounts => {
            if(accounts.length > 0) {
                if(accounts[0].balance > 0 && !accounts[0].pause) {
                    return Promise.all([
                        ...
                        _db.collection('users').find({owner: req.jwt.payload.user_id}, {_id: 0, id: 1, name: 1}).toArray()
                    ]);
                } else {
                    res.send({ status: true, balance: accounts[0].balance, paused: accounts[0].pause });
                    res.end();
                    throw new Error('negative balance or paused');
                }
            } else {
